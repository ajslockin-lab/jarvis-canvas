      ...
      <div className="relative z-10 max-w-2xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-orbitron text-lg font-bold tracking-[0.15em] text-[#00E5FF]">
                SYSTEM CONFIG
              </h1>
              <p className="font-rajdhani text-[11px] text-[#5a7a8a] tracking-wide">
                PERSONALIZE JARVIS OPERATIONS
              </p>
            </div>
          </div>
          <Link
            href="/dashboard"
            className="hud-btn px-3 py-2 flex items-center gap-2"
          >
        ...
        <SettingsSection title="DATA SYNC" icon={<RefreshCw className="w-4 h-4" />}>
          <div className="flex items-center gap-4">
            <button
              onClick={handleSync}
              disabled={syncing}
              className="hud-btn px-4 py-2.5 flex items-center gap-2 disabled:opacity-50"
            >
            ...
            {syncResult && (
              <span
                className={`font-mono-data text-[11px] font-bold ${
                  syncResult.includes("Synced") ? "text-[#00FF88]" : "text-[#FF9500]"
                }`}
              >
        ...
        <SettingsSection title="ENERGY LEVEL" icon={<Moon className="w-4 h-4" />}>
          <div className="space-y-3">
            <p className="font-rajdhani text-[13px] text-[#5a7a8a]">
              How much energy you typically have. JARVIS uses this to suggest study tasks.
            </p>
            <div className="flex items-center gap-3">
                ...
                {[1, 2, 3, 4, 5].map((level) => (
                  <button
                    ...
                    className={`h-10 transition-all ${
                      level <= energyLevel
                        ? "border border-[#00E5FF]/50 bg-[#00E5FF]/10 shadow-[0_0_8px_rgba(0,229,255,0.15)]"
                        : "border border-[#00B4FF]/10 bg-[#0A1520]/50 hover:border-[#00B4FF]/25"
                    }`}
                  >
        ...
        <div className="hud-panel p-4 mt-8">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="font-rajdhani text-[13px] text-[#5a7a8a]">
                Your Canvas token is encrypted and never leaves the server. JARVIS only reads your
                Canvas data — it never modifies anything.
              </p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="hud-panel mb-8 p-6">
      ...
      <div className="hud-section-header mb-5">
        ...
        <h2 className="font-orbitron text-[11px] font-bold tracking-[0.2em] text-[#00B4FF]">
          {title}
        </h2>
      </div>
...
function ToggleRow({
  ...
  return (
    <div className="flex items-center justify-between py-2">
      ...
      <button
        onClick={onToggle}
        className={`relative w-12 h-6 transition-colors ${enabled ? "bg-[#00B4FF]/40" : "bg-[#0A1520] border border-[#00B4FF]/10"}`}
      >
        <div
          className={`absolute top-0.5 w-5 h-5 transition-transform shadow-sm ${
            enabled ? "translate-x-6 bg-[#00E5FF]" : "translate-x-0.5 bg-[#5a7a8a]"
          }`}
        />
      </button>
    </div>
