...
// Idempotent. Refuses to run without the explicit WIPE=yes env to avoid
...
try {
  ...
  // RESTART IDENTITY resets serial sequences. CASCADE clears FK dependents.
  ...
  for (const [n, c] of counts) console.log(`wiped ${n}: ${c} rows remaining`);
  console.log("\n✓ done");
} catch (err) {
