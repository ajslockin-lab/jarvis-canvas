...
const DATABASE_URL =
  process.env.DATABASE_URL ?? "postgresql://postgres:password@127.0.0.1:54329/jarvis";
...
async function main() {
  ...
  for (let i = 0; i < 5; i += 1) {
    const m = i === 0 ? null : `filler-${i}`;
    const r = await fetchJson("/api/chat/sessions", {
      method: "POST",
      body: m ? JSON.stringify({ firstMessage: m }) : JSON.stringify({}),
    });
    if (r.status !== 201) {
      log(`filler session ${i} created`, false, `status=${r.status}`);
      break;
    }
    const fid = r.body?.session?.id;
    if (fid) createdIds.push(fid);
    await new Promise((resolve) => setTimeout(resolve, 30));
  }
  // After creating 6 (1 original + 5 filler), the original `sessionId` should have been evicted.
  ...
  const originalStillThere = sidebar.some((s) => s.id === sessionId);
  log("oldest session evicted", !originalStillThere, `original=${sessionId}`);
  const fetchEvicted = await fetchJson(`/api/chat/sessions/${sessionId}`);
  ...
  phase("T6 — wait window up to 65s and re-check");
  const stillAlive = await waitForInactivity(reminderId, 65_000);
  log(
