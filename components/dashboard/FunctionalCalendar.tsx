...
export default function FunctionalCalendar({
  ...
  const getUrgency = (dueDate: string | Date | null) => {
    ...
    const hoursLeft =
      (new Date(dueDate).getTime() - referenceDate.getTime()) / (1000 * 60 * 60);
    ...
    <div className="w-full">
      <div className="mb-4 flex items-center justify-between">
        <button
          ...
          className="hud-arrow text-[#7d99aa] hover:text-[#00E5FF]"
        ...
        {days.map((date, index) => {
          ...
          return (
            <button
              ...
              className={`
                hud-timeline-day min-h-[104px] cursor-pointer p-2 text-left transition-all duration-200
                ${todayFlag ? "today" : ""}
                ${pastFlag && !todayFlag ? "opacity-55" : ""}
              `}
            >
              <div className="mb-2 flex items-center justify-between">
                <span
                  className={`font-orbitron text-[10px] tracking-wider ${
                    todayFlag ? "font-bold text-[#00E5FF]" : "text-[#7d99aa]"
                  }`}
                >
                  {dayNames[index]}
                </span>
              ...
              <div className="flex flex-col gap-1">
                {count > 0 ? (
                  <>
                    {dayAssignments.slice(0, 2).map((assignment) => {
                      ...
                      return (
                        <div key={assignment.id} className="flex items-center gap-1">
                          <Circle
                            className={`h-1.5 w-1.5 ${urgencyColor(urgency)}`}
                            fill="currentColor"
                          />
                          <span className="font-mono-data truncate text-[9px] text-[#8aa6b7]">
                            {assignment.name}
                          </span>
                        </div>
                      );
                    ...
                    {count > 2 && (
                      <span className="font-mono-data pl-2.5 text-[9px] text-[#7d99aa]">
                        +{count - 2}
                      </span>
                    )}
                  </>
        ...
        <div className="mt-4 rounded-lg border border-[#00B4FF]/20 bg-[#0A1520]/80 p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="font-mono-data text-[10px] text-[#7d99aa] transition hover:text-[#00E5FF]"
            >
              CLOSE
            </button>
          </div>
          <div className="space-y-2">
            {getDayAssignments(selectedDay).length > 0 ? (
                ...
                return (
                  <div key={assignment.id} className="flex items-center gap-2">
                    <Circle
                      ...
                      fill="currentColor"
                    />
                    <span className="font-rajdhani text-sm text-[#e8f4f8]">
                      {assignment.name}
                    </span>
                    <span className="font-mono-data ml-auto text-[10px] text-[#7d99aa]">
              ...
              <p className="font-mono-data text-[11px] text-[#7d99aa]">
