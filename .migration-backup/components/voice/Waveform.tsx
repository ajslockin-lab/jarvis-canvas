...
export default function Waveform({ isActive, canvasRef }: WaveformProps) {
  ...
  const bars = useRef<number[]>(Array.from({ length: 40 }, (_, index) => 14 + (index % 7) * 2));
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let index = 0; index < barCount; index += 1) {
        ...
        const wave =
          Math.sin(phase + index * 0.42) * 0.55 +
          Math.sin(phase * 0.55 + index * 0.18) * 0.35;
        ...
        bars.current[index] += (target - bars.current[index]) * 0.16;

        const currentHeight = Math.max(8, Math.min(88, bars.current[index]));
