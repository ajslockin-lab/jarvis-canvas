  ...
  return (
    <div className="space-y-3">
      {visibleAlerts.map((alert) => (
        <div key={alert.id} className={`hud-alert p-4 group ${alert.urgent ? "hud-alert-urgent" : ""}`}>
          <div className="flex items-start gap-3">
            ...
            {alert.dismissable && (
              <button onClick={() => setDismissed((prev) => new Set([...prev, alert.id]))} className="p-1 text-[#5a7a8a] hover:text-[#e8f4f8] opacity-0 group-hover:opacity-100 transition">
                <X className="w-3 h-3" />
              </button>
            )}
          </div>
