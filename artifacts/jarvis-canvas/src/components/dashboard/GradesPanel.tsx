          ...
          <div key={g.name} className="group">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                {g.trend === "up" && <TrendingUp className="w-3 h-3 text-[#00FF88]" />}
                {g.trend === "down" && <TrendingDown className="w-3 h-3 text-[#FF4D4D]" />}
                {g.trend === "same" && <Minus className="w-3 h-3 text-[#5a7a8a]" />}
              ...
              <div className="flex items-center gap-2">
                <span className="font-mono-data text-xl font-bold text-[#e8f4f8]">{g.percent}</span>
                <span className={`text-[10px] font-bold px-1.5 py-0.5 ${getBadgeClass(g.percent)}`}>{getLetterGrade(g.percent)}</span>
              </div>
            </div>
            <div className="hud-bar-track">
              <div className="hud-bar-fill" style={{ width: `${Math.min(g.percent, 100)}%`, background: getBarColor(g.percent) }} />
            </div>
          </div>
        ))
      )}
