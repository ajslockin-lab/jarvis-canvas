...
export interface EmailService {
  ...
  // Distinct from verification so a user can tell them apart in their inbox
  // and the templates can use different copy.
...
class ResendEmailService implements EmailService {
  ...
  async sendPasswordResetCode(to: string, code: string): Promise<void> {
    const result = await this.client.emails.send({
      from: this.fromAddress,
      to,
      subject: `${code} — reset your Carvis password`,
      text:
        `Your Carvis password-reset code is: ${code}\n\n` +
        `This code expires in 15 minutes.\n` +
        `If you didn't request a password reset, you can ignore this email — your password is unchanged.\n`,
...
// Password-reset HTML. Same HUD palette as the verification email — only the
// copy changes. A user who just signed up should never get one of these, and
// vice versa, but we still keep the templates visually distinct enough that
// glance-recognizing the email type works.
...
<html lang="en">
  <body style="margin:0;padding:0;background:${bg};">
    <div style="display:none;max-height:0;overflow:hidden;mso-hide:all;font-size:1px;color:${bg};">
      Your Carvis password-reset code is ${code}. Expires in 15 minutes.
    </div>
          ...
          <table role="presentation" width="480" cellspacing="0" cellpadding="0" border="0" style="max-width:480px;width:100%;">
            ...
            <tr>
              <td style="background:${panel};border:1px solid ${border};border-radius:6px;padding:40px 32px;">
                ...
                <p style="margin:0 0 28px;font-family:'Helvetica Neue',Arial,sans-serif;font-size:15px;line-height:1.55;color:${textMuted};">
                  Enter this code on the reset page to choose a new password. It only works once.
                </p>
                ...
                <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:13px;line-height:1.6;color:${textDim};">
                  Didn't request this? You can safely ignore the email — your password is unchanged and the code will expire on its own.
                </p>
              </td>
            </tr>
            ...
            <tr>
              <td style="padding:24px 8px 0;text-align:center;">
                ...
                <p style="margin:0;font-family:'Helvetica Neue',Arial,sans-serif;font-size:11px;color:${textDim};">
                  You're getting this because someone (hopefully you) requested a password reset at carvis.app.
                </p>
              </td>
            </tr>
          </table>
...
/**
 * Send a password-reset code. Same swallow-on-failure contract as
 * sendVerificationCode — the user can always request a fresh code.
 */
export async function sendPasswordResetCode(to: string, code: string): Promise<void> {
  try {
    await service.sendPasswordResetCode(to, code);
  } catch (err) {
...
/**
 * Dev-only: returns the password-reset code so the route can surface it
 * in the response body. Mirrors devVerificationCodeIfEnabled.
 */
export function devPasswordResetCodeIfEnabled(code: string): string | null {
  if (process.env["RESEND_API_KEY"]) return null;
  return code;
}